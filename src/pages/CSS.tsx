...
const CSS: React.FC = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/css')
      ...
      .catch((error) => console.error('Error fetching CSS data:', error));
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      const CSSScore = calculateScore(answers);
      setScore(CSSScore);
      const percentage = (CSSScore / questions.length) * 100;
      ...
      if(username){
        addScoreToLeaderboard(username, CSSScore, Language);
      }
    }, 5000);
  };
