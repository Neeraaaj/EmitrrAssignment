...
const JS: React.FC = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/JS')
      .then((response) => response.json())
      .then((data: Question[]) => setQuestions(data))
      .catch((error) => console.error('Error fetching JS data:', error));
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      const JSScore = calculateScore(answers);
      setScore(JSScore);
      const percentage = (JSScore / questions.length) * 100;
      ...
      if(username){
        addScoreToLeaderboard(username, JSScore, Language);
      }
    }, 5000);
  };
